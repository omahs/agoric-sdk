  ...
  */
  ...
  function revivePromise(slot) {
    ...
    if (val) {
      ...
      // after buildRootObject(), which is given the deserialized
      // vatParameters. The only way revivePromise() might encounter a
      // pre-existing vpid is if these vatParameters include a promise that the
      // previous incarnation watched, but that `buildRootObject` in the new
      // incarnation didn't explicitely watch again. This can be either
      // a previously imported promise, or a promise the previous incarnation
      // exported, regardless of who the decider now is.
      // In that case convertSlotToVal() has already deserialized the vpid, but
      // since `buildRootObject` didn't explicitely call watchPromise on it,
      // no registration exists so loadWatchedPromiseTable attempts to revive
      // the promise.
      return val;
    }
