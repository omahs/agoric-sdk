 ...
 * much" and "of what kind". Amounts are values labeled with a brand. AmountMath
 * executes the logic of how amounts are changed when digital assets are merged,
 * separated, or otherwise manipulated. For example, a deposit of 2 bucks into a
 * purse that already has 3 bucks gives a new purse balance of 5 bucks. An empty
 * purse has 0 bucks. AmountMath relies heavily on polymorphic MathHelpers,
 * which manipulate the unbranded portion.
 */
 ...
 * a set of whatever asset each key represents. A `Key` is a passable value that
 * can be used as an element in a set (SetStore or CopySet) or as the key in a
 * map (MapStore or CopyMap).
 *
 ...
 * once or more times, i.e., some positive bigint number of times, representing
 * that quantity of the asset represented by that key.
 */
export type AmountValue =
  | bigint
  | SetValue
  | CopySet
  | import('@endo/patterns').CopyBag<any>;
  ...
  : K extends 'set'
    ...
    : K extends 'copySet'
      ...
      : K extends 'copyBag'
        ? import('@endo/patterns').CopyBag<any>
        : never;
export type AssetKindForValue<V extends AmountValue> = V extends bigint
  ? 'nat'
  : V extends SetValue
    ? 'set'
    : V extends CopySet
...
export type DisplayInfo<K extends AssetKind = AssetKind> = {
   ...
   * to the left, or in other words, which position corresponds to whole
   * numbers. We require fungible digital assets to be represented in integers,
   * in the smallest unit (i.e. USD might be represented in mill, a thousandth
   * of a dollar. In that case, `decimalPlaces` would be 3.) This property is
   * optional, and for non-fungible digital assets, should not be specified. The
   * decimalPlaces property should be used for _display purposes only_. Any
   * other use is an anti-pattern.
   */
  decimalPlaces?: number | undefined;
  /**
   * - the kind of asset, either AssetKind.NAT (fungible) or AssetKind.SET or
   *   AssetKind.COPY_SET (non-fungible)
   */
  assetKind: K;
};
/**
 * The brand identifies the kind of issuer, and has a function to get the
 * alleged name for the kind of asset described. The alleged name (such as 'BTC'
 * or 'moola') is provided by the maker of the issuer and should not be trusted
 * as accurate.
 *
 ...
 * represents the issuer they intended, since the same brand can be reused by a
 * misbehaving issuer.
 */
 ...
 */
export type IssuerIsLive = (
  payment: ERef<Payment<AssetKind>>,
) => Promise<boolean>;
/**
 * Get the amount of digital assets in the payment. Because the payment is not
 * trusted, we cannot call a method on it directly, and must use the issuer
 * instead.
 *
 * If the payment is a promise, the operation will proceed upon fulfillment.
 */
export type IssuerGetAmountOf<K extends AssetKind> = (
  payment: ERef<Payment<AssetKind>>,
) => Promise<Amount<K>>;
/**
 * Burn all of the digital assets in the payment. `optAmountShape` is optional.
 * If the `optAmountShape` pattern is present, the amount of the digital assets
 * in the payment must match `optAmountShape`, to prevent sending the wrong
 ...
 * `Pattern`. But in optional pattern position, a top level `undefined` will be
 * interpreted as absence. If you want to express a `Pattern` that will match
 * only `undefined`, use `M.undefined()` instead.
 */
 ...
 * combining payments, burning payments, and claiming payments exclusively). The
 * issuer should be gotten from a trusted source and then relied upon as the
 * decider of whether an untrusted payment is valid.
 */
export type Issuer<K extends AssetKind = AssetKind> = {
  /**
   * Get the Brand for this Issuer. The Brand indicates the type of digital
   * asset and is shared by the mint, the issuer, and any purses and payments of
   * this particular kind. The brand is not closely held, so this function
   * should not be trusted to identify an issuer alone. Fake digital assets and
   * amount can use another issuer's brand.
   */
  getBrand: () => Brand<K>;
...
export type PaymentLedger<K extends AssetKind = AssetKind> = {
  mint: Mint<K>;
  /**
   * Externally useful only if this issuer uses recovery sets. Can be used to
   * get the recovery set associated with minted payments that are still live.
   */
...
/**
 * Holding a Mint carries the right to issue new digital assets. These assets
 * all have the same kind, which is called a Brand.
 */
...
export type DepositFacet = {
   ...
   * returning the amount. If the optional argument `optAmount` does not equal
   * the amount of digital assets in the payment, throw an error.
   *
   * If payment is a promise, throw an error.
   */
  receive: DepositFacetReceive;
};
export type PurseDeposit<K extends AssetKind> = (
  payment: Payment<K>,
  optAmountShape?: Pattern,
) => Amount<K>;
/**
...
export type Purse<K extends AssetKind = AssetKind> = {
  ...
  /**
   * Deposit all the contents of payment into this purse, returning the amount.
   * If the optional argument `optAmount` does not equal the amount of digital
   ...
   * are the payments that can still be recovered in emergencies by, for
   * example, depositing into this purse. Such a deposit action is like
   * canceling an outstanding check because you're tired of waiting for it. Once
   * your cancellation is acknowledged, you can spend the assets at stake on
   * other things. Afterwards, if the recipient of the original check finally
   * gets around to depositing it, their deposit fails.
   *
   ...
   * upgrade. This deposits all the payments in this purse's recovery set into
   * the purse itself, returning the total amount of assets recovered.
   *
 ...
 * claimed (getting an exclusive payment). Payments are linear, meaning that
 * either a payment has the same amount of digital assets it started with, or it
 * is used up entirely. It is impossible to partially use a payment.
 *
 * Payments are often received from other actors and therefore should not be
 * trusted themselves. To get the amount of digital assets in a payment, use the
...
export type Payment<K extends AssetKind = AssetKind> = {
  /**
   * Get the allegedBrand, indicating the type of digital asset this payment
   * purports to be, and which issuer to use. Because payments are not trusted,
   * any method calls on payments should be treated with suspicion and verified
   * elsewhere.
   */
  getAllegedBrand: () => Brand<K>;
};
 ...
 * reduced to the behavior of the math on values. We extract this custom logic
 * into mathHelpers. MathHelpers are about value arithmetic, whereas AmountMath
 * is about amounts, which are the values labeled with a brand. AmountMath use
 * mathHelpers to do their value arithmetic, and then brand the results, making
 * a new amount.
 *
...
export type MathHelpers<V extends AmountValue> = {
  ...
  /**
   * Return what remains after removing the right from the left. If something in
   * the right was not in the left, we throw an error.
   */
